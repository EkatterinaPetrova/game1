...
def get_text_messages(bot, cur_user, message):
    ...
    elif ms_text == "Прислать предсказание":
        bot.send_message(chat_id,text=get_prediction())

    elif ms_text == "Прислать фильм":
        send_film(bot, chat_id)
    elif ms_text == "Прислать погоду":
        keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
        keyboard.add(types.KeyboardButton("Нажми, чтобы отправить", request_location=True))
        bot.send_message(chat_id, text="Отправь мне геолокацию, чтобы я отправил погоду", reply_markup=keyboard)
...
def get_weather(message):
    res = requests.get(
        f"http://api.weatherapi.com/v1/forecast.json?key=472ebd4f9c4a46bdaaf105151222605&q={message.location.latitude},{message.location.longitude}&aqi=no").json()
    return res
