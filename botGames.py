...
def draw_canvas():
    ...
    with io.BytesIO() as byt:
        img.save(byt, 'png')
        image_bytes = byt.getvalue()
    return image_bytes


def random_word():
    with open(f"{Path(__file__).parent.absolute()}\\words.txt", "r", encoding="UTF-8") as text:
        return random.choice(list(map(str, text.read().split())))
...
class Draw_Game:
    def __init__(self, id, name):
        ...
        self.word = ""
        self.round = 0
        setattr(self, str(id), {"score": 0, "bool": True})
        activeGames.update({id: self, name: self})
        print(getattr(activeGames[id], "users"))

    def new_user(self, message, bot):
        for user in self.users:
            bot.send_message(user, f"К нам присоединился {message.from_user.username}")
        bot.send_message(message.from_user.id, "Вы вошли в лобби, ждите")
        self.users += (message.from_user.id,)
        setattr(self, str(message.from_user.id), {"score": 0, "bool": True})
        print(getattr(self, str(message.from_user.id)))
        activeGames[message.from_user.id] = self
        print(activeGames)

    def start_round(self, bot):
        self.word = random_word()
        if self.round >= len(self.users) - 1:
            for user in self.users:
                bot.send_message(user, f"Игра окончена! {'тут надо вывести победителя'}")
                break
        active_user = self.users[self.round]
        for users in self.users:
            user = getattr(self, str(users))
            user["bool"] = users != active_user
            setattr(self, str(users), user)
            print(user)
        bot.send_photo(active_user, photo=draw_canvas(),
                       caption=f"Сейчас твоя очередь рисовать, держи холст!\nТвое слово {self.word}")
        self.round += 1

    def forward(self, bot, message):
        for user in self.users:
    ...
    def check_answer(self, bot, message):
        print("чек работает")
        if message.text.lower() != self.word:
            self.forward(bot, message)
        else:
            user = getattr(self, str(message.chat.id))
            user['score'] += 1
            setattr(self, str(message.chat.id), user)
            print("тут тоже")
            for user in self.users:
                bot.send_message(user, f"Это было слово {self.word}, {message.from_user.username} получает одно очко")
            self.start_round(bot)
...
def callback_worker(bot, cur_user, cmd, par, call):
    ...
    if cmd == "newGame":
        ...
        newGame(chat_id, GameNIM_Multiplayer(bot, cur_user))
...
def get_text_messages(bot, cur_user, message):
    ...
    # ======================================= реализация игры Крокодил Multiplayer

    elif ms_text == "Крокодил":
        id = uuid.uuid1().hex
        locals()[id] = Draw_Game(message.from_user.id, id)
        keyboard = types.InlineKeyboardMarkup()
        keyboard.add(types.InlineKeyboardButton("Начать игру", callback_data="start_game"))
        bot.send_message(message.from_user.id, text=f"Лобби {id} создано", reply_markup=keyboard)
